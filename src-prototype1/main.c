...
{
   RAM_BANK = (MAP_RAM_BANK_START); // used to be setBank()
...
{
   RAM_BANK = (SPRITE_RAM_BANK);  // used to be setBank()
...
//unsigned getShipAddr(byte index)
//{
...
void move()
{
   if (kbhit())
      switch(cgetc())
      {
         ...
         case 'b': // build settlement at current center
         {
            byte cx, cy;
            byte terr;
            
            map_get_center_tile(&cx, &cy);
            terr = map_get_terrain_at(cx, cy);
            ...
            else if (map_has_object_at(cx, cy))
            {
               gotoxy(5,55);
               cprintf("Cannot build at (%02x,%02x): Occupied!  ", cx, cy);
            }
            else if (map_has_settlement_nearby(cx, cy, 5))
            {
               gotoxy(5,55);
...
void main()
{
   ...
   shipIndex = 7; // Genoese for now.  Otherwise: rand() % 8;
